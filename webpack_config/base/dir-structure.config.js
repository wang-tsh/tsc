...
moduleExports.especialDir = path.resolve(moduleExports.srcRootDir, './especial'); // 存放各个页面独有的部分，如入口文件、只有该页面使用到的css、模板文件等
moduleExports.resourceDir = path.resolve(moduleExports.srcRootDir, './resource'); // 存放各个页面使用到的公共资源
moduleExports.logicDir = path.resolve(moduleExports.resourceDir, './common'); // 存放公用的业务逻辑
...
moduleExports.componentsDir = path.resolve(moduleExports.resourceDir, './components'); // 存放组件，可以是纯HTML，也可以包含js/css/image等，看自己需要
